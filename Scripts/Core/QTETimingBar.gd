...
static func run(
	...
	qte.bf = parent_bf
	qte.sweep_duration = sweep_ms / 1000.0
...
func _process(delta: float) -> void:
	...
	var bar_w = 520.0 # From bar_bg layout
	cursor.position.x = t * (bar_w - cursor.size.x)
